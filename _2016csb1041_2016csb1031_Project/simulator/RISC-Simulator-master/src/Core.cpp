 ...
 *
 * Authors: Naman Goyal (email: 2015csb1021@iitrpr.ac.in) / Prasad Kshirsagar (email: 2016csb1041@iitrpr.ac.in)
 */
...
void Core::decode() {
	...
	if( !(opcode5 == 1 || ( opcode5 == 0 && opcode3 == 1 && opcode1 == 1 && ( opcode4 == 1 || opcode2 == 0) ) ) 	|| 	(opcode5 == 1 && opcode4 == 0 && opcode3 == 0 && opcode2 == 1 && opcode1 == 1) || (opcode5 == 1 && opcode4 == 0 && opcode3 == 1 && opcode2 == 0 && opcode1 == 1) || (opcode5 == 1 && opcode4 == 0 && opcode3 == 1 && opcode2 == 1 && opcode1 == 0) || (opcode5 == 1 && opcode4 == 0 && opcode3 == 1 && opcode2 == 1 && opcode1 == 1) || (opcode5 == 1 && opcode4 == 1 && opcode3 == 0 && opcode2 == 0 && opcode1 == 0) || (opcode5 == 1 && opcode4 == 1 && opcode3 == 0 && opcode2 == 0 && opcode1 == 1) || (opcode5 == 1 && opcode4 == 1 && opcode3 == 0 && opcode2 == 1 && opcode1 == 0) || (opcode5 == 1 && opcode4 == 1 && opcode3 == 0 && opcode2 == 1 && opcode1 == 1) || (opcode5 == 1 && opcode4 == 1 && opcode3 == 1 && opcode2 == 0 && opcode1 == 0)  || (opcode5 == 1 && opcode4 == 1 && opcode3 == 1 && opcode2 == 0 && opcode1 == 1)){
		temp_isWb = true;
		//pprint(2)<<"isWb ";
	}
	else{
	...
	else if((opcode5 == 0 && opcode4 == 1 && opcode3 == 1 && opcode2 == 1) || (opcode5 == 1 && opcode4 == 1 && opcode3 == 1 && opcode2 == 0 && opcode1 == 1) || (opcode5 == 1 && opcode4 == 1 && opcode3 == 1 && opcode2 == 1 && opcode1 == 0) ){
		temp_isAdd = true;
		//pprint(2)<<"isAdd ";
	}
	else{
	...
	unsigned int temp_operand2;
        unsigned long long int vtemp_operand1;
	unsigned long long int vtemp_operand2;
	...
	if (temp_isRet){
        ...
        else if (temp_isVadd || temp_isVsub || temp_isVmul || temp_isVdiv || temp_isVmod || temp_isVand ){ 
		vtemp_operand1 = V[temp_rs1];
		//pprint(2)<<"Operand1: "<<dec<<temp_operand1<<" (Read from ra OR R15)"<<endl;
		fprint(1)<<"; "<<vectorstring(temp_rs1)<<" = 0x";
                fprint(1)<<hex<<vtemp_operand1;
	}
        else if (temp_isVld || temp_isVst){
	...
	if (temp_isSt){
        ...
        if (!temp_isImmediate && (temp_isVadd || temp_isVsub || temp_isVmul || temp_isVdiv || temp_isVmod || temp_isVand || temp_isVmov2)){
		fprint(1)<<hex<< vtemp_operand2;
	}
	else if (!temp_isImmediate ){
	...
	unsigned int temp_A = temp_operand1;
        unsigned long long int vtemp_A = vtemp_operand1;
	//pprint(2)<<"A: "<<dec<<temp_A<<" (operand1)"<<endl;

	unsigned int temp_B;
        unsigned long long int vtemp_B;
	...
	of_ex.operand2.Write(temp_operand2);
        of_ex.VA.Write(vtemp_A);
	of_ex.VB.Write(vtemp_B);
	of_ex.Voperand2.Write(vtemp_operand2);
...
void Core::execute() {
	...
	unsigned int temp_operand2 = of_ex.operand2.Read();
        unsigned long long int vtemp_A = of_ex.VA.Read();
	unsigned long long int vtemp_B = of_ex.VB.Read();
	unsigned long long int vtemp_operand2 = of_ex.Voperand2.Read();
	...
	unsigned int temp_aluResult = 0;
        unsigned long long int vtemp_aluResult = 0;
	...
	if (temp_isAdd){
        ...
        if (temp_isVadd){
		//pprint(2)<<"ADD operation"<<endl;
		//vtemp_aluResult = temp_A + temp_B;
                  unsigned short int A1,A2,A3,A4,B1,B2,B3,B4,ans1,ans2,ans3,ans4;
                  A1 = vtemp_A & 0xFFFF;
                 // cout << A1 <<endl;
                  A2 = (vtemp_A >> 16) & 0xFFFF;
                  //cout << A2 <<endl;
                  A3 = (vtemp_A >> 32) & 0xFFFF;
                  //cout << A3 <<endl;
                  A4 = (vtemp_A >> 48) & 0xFFFF;
                  //cout << A4 <<endl;
                  B1 = vtemp_B & 0xFFFF;
                  B2 = (vtemp_B >> 16) & 0xFFFF;
                  B3 = (vtemp_B >> 32) & 0xFFFF;
                  B4 = (vtemp_B >> 48) & 0xFFFF;
                  ans1 = A1 + B1;
                 // cout << ans1 <<endl;
                  ans2 = A2 + B2;
                  //cout << ans2 <<endl;
                 ...
                 // cout << ans3 <<endl;
                  ...
                  vtemp_aluResult = ans4;
                  //cout << vtemp_aluResult <<endl;
                  vtemp_aluResult = (vtemp_aluResult << 16) | ans3 ;
                   //cout << vtemp_aluResult <<endl;
                  vtemp_aluResult = (vtemp_aluResult << 16) | ans2 ;
                    //cout << vtemp_aluResult <<endl;
                  vtemp_aluResult = (vtemp_aluResult << 16) | ans1 ;
                    //cout << vtemp_aluResult <<endl;
	...
	if (temp_isCmp){
		...
		//vtemp_aluResult = temp_A + temp_B;
                 ...
                 // cout << A1 <<endl;
                  ...
                  B3 = (vtemp_B >> 32) & 0xFFFF;
                  B4 = (vtemp_B >> 48) & 0xFFFF;
                  ans1 = (unsigned short int)( (signed int)A1 * (signed int)B1 );
                 // cout << ans1 <<endl;
                  ans2 = (unsigned short int)( (signed int)A2 * (signed int)B2 );
                  //cout << ans2 <<endl;
                  ans3 = (unsigned short int)( (signed int)A3 * (signed int)B3 );
                 // cout << ans3 <<endl;
                  ans4 = (unsigned short int)( (signed int)A4 * (signed int)B4 );
                  //cout << ans4 <<endl;
                   if(temp_isImmediate)
                    {
                        ans1 = (unsigned short int)( (signed int)A1 * (signed int)vtemp_B );
                        ans2 = (unsigned short int)( (signed int)A2 * (signed int)vtemp_B );
                        ans3 = (unsigned short int)( (signed int)A3 * (signed int)vtemp_B );
                        ans4 = (unsigned short int)( (signed int)A4 * (signed int)vtemp_B );
	...
	if (temp_isMov){
		...
		vtemp_aluResult = (((unsigned long long int)temp_A) << 32) | temp_B;
                
	}
        if (temp_isVmov2){
		
		vtemp_aluResult =  vtemp_B;
	...
	ex_ma.aluResult.Write(temp_aluResult);
        ex_ma.ValuResult.Write(vtemp_aluResult);
	ex_ma.operand2.Write(temp_operand2);
        ex_ma.Voperand2.Write(vtemp_operand2);
...
void Core::mem_access() {
	...
	unsigned int temp_aluResult = ex_ma.aluResult.Read();
        unsigned long long int vtemp_aluResult = ex_ma.ValuResult.Read();
	unsigned int temp_operand2 = ex_ma.operand2.Read();
        unsigned long long int vtemp_operand2 = ex_ma.Voperand2.Read();
	...
	unsigned int temp_mdr = temp_operand2;
        unsigned long long int vtemp_mdr = vtemp_operand2;

	unsigned int temp_ldResult;
        unsigned long long int vtemp_ldResult;
	...
	if (temp_isLd){
		...
		//pprint(2)<<"Reading from Memory at address 0x"<<hex<<temp_aluResult<<endl;
		if (temp_mar >= 0 && temp_mar <= MEM_CAPACITY - sizeof(unsigned int)){
			vtemp_ldResult = MEM.Read(temp_mar);
                        //cout <<temp_mar <<endl;
                       // cout <<vtemp_ldResult <<endl;
		}
	...
	ma_rw.ldResult.Write(temp_ldResult);
        ma_rw.VldResult.Write(vtemp_ldResult);
	ma_rw.aluResult.Write(temp_aluResult);
        ma_rw.ValuResult.Write(vtemp_aluResult);
...
void Core::write_back() {
	...
	unsigned int temp_ldResult = ma_rw.ldResult.Read();
        unsigned long long int vtemp_ldResult = ma_rw.VldResult.Read();
	unsigned int temp_aluResult = ma_rw.aluResult.Read();
        unsigned long long int vtemp_aluResult = ma_rw.ValuResult.Read();
	...
	unsigned int temp_result;
        unsigned long long int vtemp_result;
	unsigned int temp_addr;
        unsigned int x,y;
        unsigned int temp_addr1;
        unsigned int temp_addr2;
	...
	if (temp_isWb){
		...
		if (temp_isLd){
			...
			vtemp_aluResult = vtemp_ldResult;
			//pprint(2)<<"Writing data "<<dec<<temp_result;
			//pprint(2)<<" (ldResult)";
		}
		else if (temp_isCall){
		...
		if (temp_isCall){
			temp_addr = 15;
                ...
                {
                      temp_addr1 = inst_bitset(temp_instruction_word,19,22);
                      temp_addr2 = inst_bitset(temp_instruction_word,15,18);
		...
		//pprint(2)<<" to register R"<<dec<<temp_addr<<endl;
               ...
               // cout <<R[1] <<endl;
                if(temp_isVmov1 || temp_isVadd || temp_isVsub || temp_isVmul || temp_isVdiv || temp_isVmod || temp_isVand || temp_isVld)
                V[temp_addr] = vtemp_aluResult;
                else if(temp_isVmov2)
                  {
                        
                       x = (unsigned int)(( vtemp_aluResult) >> 32);
                      // cout <<temp_addr1 << endl;
                       y = (unsigned int)(vtemp_aluResult);
                        //cout << temp_addr2 << endl;
                       R[temp_addr1] = x; 
                       R[temp_addr2] = y;
                ...
                if(temp_isVmov1 || temp_isVadd || temp_isVsub || temp_isVmul || temp_isVdiv || temp_isVmod || temp_isVand || temp_isVld)
                {
                fprint(1)<<"; "<<vectorstring(temp_addr)<<" = 0x"<<hex<<vtemp_aluResult;
                }
                else if(temp_isVmov2)
                {
                  
                  fprint(1)<<"; "<<registerstring(temp_addr1)<<" = 0x"<<hex<<x;
                  fprint(1)<<"; "<<registerstring(temp_addr2)<<" = 0x"<<hex<<y;
...
bool Core::check_data_conflict(PipelineRegister& A, PipelineRegister& B){
	...
	if (B_opcode5 == 0 && B_opcode4 == 1 && B_opcode3 == 1 && B_opcode2 == 1 && B_opcode1 == 1){
        ...
        if (B_opcode5 == 1 && B_opcode4 == 1 && B_opcode3 == 1 && B_opcode2 == 1 && B_opcode1 == 0){
		return false;	//B is vst
	}
...
string Core::disassemble (unsigned int inst_word){
	...
	if (opcode5 == 0 && opcode4 == 0 && opcode3 == 0 && opcode2 == 0 && opcode1 == 1){
        ...
        if (opcode5 == 1 && opcode4 == 0 && opcode3 == 1 && opcode2 == 1 && opcode1 == 0){
		inst = "VMOV2" + modifier + " " + vectorstring(rd) + ", " + registerstring(rs1);
		if (isImmediate){
			inst += ", " + hexstring(imm);
		}
		else {
