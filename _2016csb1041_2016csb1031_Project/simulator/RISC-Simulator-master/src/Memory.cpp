...
unsigned long long int Memory::Read(unsigned int address) const
{
    if(address >= 0 && address <= capacity - sizeof(unsigned int))
    {
        unsigned long long int *add = (unsigned long long int *)(start_Add + address);
        return *add;
    }
}

void Memory::Write(unsigned int address, unsigned long long int data)
{
    if(address >= 0 && address <= capacity - sizeof(unsigned int))
    {
        unsigned long long int *add = (unsigned long long int *)(start_Add + address);
