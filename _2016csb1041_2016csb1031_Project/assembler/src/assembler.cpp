...
void Assembler::assemblerPass2(char* output) {
    ...
    for(i=0; i < mInstList.size(); i++) {
        ...
        if(inst->opcode == END) {
            ...
            if(inst->opcode == 21)
              inst->encoding = generateEncoding(inst->opcode, destReg, src2, src2+1, imm); 
            else if(inst->opcode == 22)
              inst->encoding = generateEncoding(inst->opcode, src2,destReg, destReg+1, imm); 
            else
